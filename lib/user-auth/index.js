        ...
        const html = await routerBuilder.createHtmlRouter(async ({ router }) => {
            router.get(this._validateRoute(), async (req, res) => {
                try {
                    ...
                    this._renderSimpleMessage(routerBuilder.server, req, res, messages_1.messages.success, messages_1.messages.user_validated);
                }
                catch (err) {
                    this._renderSimpleMessage(routerBuilder.server, req, res, messages_1.messages.error, err.message);
                }
            });
