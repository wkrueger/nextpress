...
class UserAuth {
    constructor(ctx) {
        ...
        this.checkSession = (req) => __awaiter(this, void 0, void 0, function* () {
            if (!req.nextpressAuth)
                throw ono({ statusCode: 401 }, "Unauthorized");
            let user = yield req.nextpressAuth.getUser();
            if (!user)
                throw ono({ statusCode: 401 }, "Unauthorized");
            //res.setHeader("X-User-Id", req.session.user.id)
            //res.setHeader("X-User-Email", req.session.user.email)
        });
        this.throwOnUnauthMw = (req, res, next) => __awaiter(this, void 0, void 0, function* () {
            try {
                yield this.checkSession(req, res, next);
                next();
            }
            catch (err) {
                next(err);
    ...
    userRoutes(routerBuilder) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const json = yield routerBuilder.opinionatedJsonRouter(({ route, yup }) => ({
                "/createUser": route({
                ...
                "/login": route({
                    ...
                    const token = yield req.nextpressAuth.setUser({ email: user.email, id: user.id });
                ...
                "/logout": route({
                    ...
                    if (req.nextpressAuth)
                        yield req.nextpressAuth.logout();
                    return { status: "OK" };
                }))
            }));
            const html = yield routerBuilder.createHtmlRouter(({ router }) => __awaiter(this, void 0, void 0, function* () {
                router.get(this._validateRoute(), router_builder_1.RouterBuilder.createHandler((req, res) => __awaiter(this, void 0, void 0, function* () {
