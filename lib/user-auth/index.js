...
class UserAuth {
    ...
    async checkAndUpdateUserRequestCap(userId, seconds) {
        if (!userId)
            throw ono({ code: "INVALID_INPUT" }, messages_1.messages.invalid_input);
        ...
        if (!lastReq ||
            ...
            throw ono({ code: "TRY_AGAIN_IN_SECONDS" }, messages_1.messages.try_again_in_a_few_seconds);
        }
    }
    ...
    async sendNewValidationEmail(userId) {
        ...
        if (!user)
            throw ono({ code: "USER_NOT_FOUND" }, messages_1.messages.user_not_found);
    ...
    async validateHash(hash) {
        if (!hash)
            throw ono({ code: "INVALID_HASH" }, messages_1.messages.invalid_hash);
    ...
    async validateLogin(inp) {
        ...
        if (user.validationHash)
            throw ono({ code: "VALIDATE_EMAIL_FIRST" }, messages_1.messages.validate_email_first);
    ...
    async performResetPwd(inp) {
        ...
        if (!found)
            throw ono({ code: "INVALID_REQUEST" }, messages_1.messages.invalid_request);
        if (inp.pwd1 !== inp.pwd2)
            throw ono({ code: "PASSWORD_CONFIRMATION_FAILED" }, messages_1.messages.password_confirmation_failed);
    ...
    userJsonMethods(helper) {
        ...
        return {
            ...
            "/request-password-reset": route({
                ...
                if (!user)
                    throw ono({ code: "NOT_FOUND" }, messages_1.messages.not_found);
