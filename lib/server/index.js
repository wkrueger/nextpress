...
class Server {
    ...
    getNextApp() {
        if (!this._nextApp) {
            ...
            var prev404 = this._nextApp.render404.bind(this._nextApp);
            var builder = new router_builder_1.RouterBuilder(this);
            var error404 = ono({ statusCode: 404 }, "Path not found (404).");
            this._nextApp.render404 = function (req, res, parsedUrl) {
                return new Promise(resolve => {
                    if (req.accepts("application/json")) {
                        builder.jsonErrorHandler(error404, req, res, () => { });
                        resolve();
                    }
                    else {
                        prev404(req, res, parsedUrl).then(resolve);
                    }
                });
            };
